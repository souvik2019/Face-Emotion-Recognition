...
Conv2D0 -> Conv2D1 -> Conv2D2 -> Batchnormalization0 -> Maxpool0 -> Dropout -> Cov2D3 -> Conv2D4 -> batchnormalization -> Maxpool1 -> Dropout -> Conv2D5 -> Conv2d6 -> Batchnormalization -> Maxpool2 -> Dropout -> FC0 -> Dropout -> FC1 ->Softmax

**Note** - (i) First arguement in ZeroPadding2D is pad size.

(ii) First, second arguement in Conv2D is number of filters and filter size.

(iii) In the arguement in *Avtivation* , function *relu* is used.

(iv) Arguement in *MaxPooling2D* is pool size
...
history=face_model.fit(x=X_train, y=y_train, epochs=50, batch_size=64,verbose=1, validation_data=(X_val,y_val) )

"""Model is run two times with *epoch=50*, so total *epoch=100*"""

preds=face_model.evaluate(x=X_test, y= (np.arange(7)==y_test[:,None]).astype(np.float32))
print('loss = '+str(preds[0]))
print('test accuracy = '+str(preds[1]))

"""We train the model using 100 *epochs* with *mini batch* size = 64. We get the training accuracy 86% and validation accuracy 66%.This model is pretty good among all the model which we have used behind the scenes.
...
img = image.load_img('/content/example.jpg', grayscale=True, target_size=(48, 48));
...
x = np.array(x, 'float32')
x = x.reshape([48, 48]);
show_img=image.load_img('/content/example.jpg', grayscale=False, target_size=(200, 200))
m=0.000000000000000000001
a=custom[0]
for i in range(0,len(a)):
    if a[i]>m:
...
img = image.load_img('/content/example22.jpeg', grayscale=True, target_size=(48, 48));
...
x = np.array(x, 'float32')
x = x.reshape([48, 48]);
show_img=image.load_img('/content/example22.jpeg', grayscale=False, target_size=(200, 200))
m=0.000000000000000000001
a=custom[0]
for i in range(0,len(a)):
    if a[i]>m:
...
img = image.load_img('/content/example2.jpg', grayscale=True, target_size=(48, 48));
...
x = np.array(x, 'float32')
x = x.reshape([48, 48]);
show_img=image.load_img('/content/example2.jpg', grayscale=False, target_size=(200, 200))
m=0.000000000000000000001
a=custom[0]
for i in range(0,len(a)):
    if a[i]>m:
...
test_pred = np.argmax(face_model.predict(X_test), axis=1)
...
Conv2D0 -> Conv2D1 -> Conv2D2 -> Batchnormalization0 -> Maxpool0 -> Dropout -> Cov2D3 -> Conv2D4 -> batchnormalization -> Maxpool1 -> Dropout -> Conv2D5 -> Conv2d6 -> Batchnormalization -> Maxpool2 -> Dropout -> FC0 -> Dropout -> FC1 ->Softmax
is giving 65% accuracy on our test data.So far with this dataset people working all over the world have been able to achieve 71% accuracy on test data.As a beginner we are close to it and further we can improve our accuracy 

(i)Experimenting different dropouts rate

(ii)Using different optimizer like RMS prop,Adamax
"""
